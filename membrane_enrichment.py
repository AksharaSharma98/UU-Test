...
while traj.nextFrame():
    ...
    for l in range(2):
        ...
        for i in range(len(leaflet_sel[l])):
            ...
            np.savetxt(os.path.join(dir,"CV Data",leaflet_label[l]+'_leaflet_enrichment_'+labels[i]+'_'+str(frame)+'.txt'), enr.flatten(), fmt='%.8e')
...
for l in range(2):
    for i in range(len(leaflet_sel[l])):
        np.savetxt(os.path.join(dir,"CV Data",leaflet_label[l]+'_leaflet_enrichment_'+labels[i]+'.txt'), hist[l][i], fmt='%.8e')
